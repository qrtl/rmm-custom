...
class ResUsers(models.Model):
    ...
    def _compute_is_intake_evaluation_user(self):
        for user in self:
            user.is_intake_evaluation_user = user.has_group("product_intake_evaluation.group_intake_evaluation_user")
    ...
    def _inverse_is_intake_evaluation_user(self):
        ...
        for user in self:
            if user.is_intake_evaluation_user:
                ...
                user.partner_id.is_intake_evaluation_branch = False

    # --- Intake Evaluation Manager ---
    def _compute_is_intake_evaluation_manager(self):
        for user in self:
            user.is_intake_evaluation_manager = user.has_group("product_intake_evaluation.group_intake_evaluation_manager")

    def _inverse_is_intake_evaluation_manager(self):
        group = self.env.ref("product_intake_evaluation.group_intake_evaluation_manager")
        for user in self:
